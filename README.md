...
const [component1] = await xyapi.assert_components([
  [component_external_id, name]
])
const [schema1] = await xyapi.assert_schemas([
  [schema_external_id, name, applies_to, components]
])
const [unit1] = await xyapi.assert_units([
  [unit_external_id, name, type, schema_id, outer_id = null]
])
const [location1] = await xyapi.assert_locations([
  [location_external_id, name, within_location_entry_id = null]
])
const [entity1] = await xyapi.assert_entities([
  [entity_external_id, name, within_entity_entry_id = null]
])
